...
const MusicCard: React.FC = () => {
    ...
    const playPrev = () => {
        const prev = (currentIdx - 1 + (songData as Song[]).length) % (songData as Song[]).length;
        setCurrentIdx(prev);
    };
        ...
        <div className="group  w-full h-full flex flex-col items-start justify-start gap-3">
            <div className="w-full rounded-lg overflow-hidden">
                <img
                    src={current.cover}
                    alt={current.title}
                    className={`w-full h-full object-cover transition-all duration-500 ${isPlaying ? "animate-vibe saturate-100" : "saturate-0"
                        }`}
                />
            </div>
            <div className="flex flex-col items-center justify-center gap-2 w-full">
                <div className="w-full flex flex-col items-center justify-center px-4 text-primary ">
                    <p className="text-md md:text-2xl ">{current.title}</p>
