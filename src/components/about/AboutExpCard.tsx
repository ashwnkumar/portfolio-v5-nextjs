            ...
            <div className="w-full flex flex-col gap-4 overflow-auto">
                {admin.workExp.map((item) => {
                    ...
                    return (
                        <div
                            key={item.id}
                            className={`w-full rounded-lg cursor-pointer transition-all duration-300 ease-in-out overflow-hidden ${isSelected ? "bg-brand-faded" : "bg-bg"}`}
                        >
                            <button
                            ...
                            >
                                <div className="flex flex-col">
                                    <h4 className={`text-lg font-semibold ${isSelected ? 'text-brand' : 'text-primary'}`}>
                                        {item.company}
                                    </h4>
