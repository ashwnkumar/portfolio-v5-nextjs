...
const Button = ({
    ...
    const btnVariants: Record<NonNullable<ButtonProps["variant"]>, string> = {
        ...
        icon: "!rounded-full p-1 bg-transparent text-brand hover:bg-brand/20",
        outline:
            "border border-brand text-brand hover:bg-brand hover:text-bg",
    };

    const commonClasses =
        "group px-4 py-2 flex items-center justify-center font-medium active:scale-95 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed";
